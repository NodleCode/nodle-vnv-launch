...
export function startNode(
	...
	if (basePath) {
		args = args.concat("--base-path=" + basePath + "/" + name.toLowerCase());
	} else {
	...
	if (key) {
		// Add bootnode keys
		args = args.concat("--node-key=" + key);
		console.log(`Added Bootnode key :: ${key}`);
	}

	if (bootnodes) {
		// Add bootnode path
		args = args.concat("--bootnodes=" + bootnodes);
		console.log(`Added Bootnode Path :: ${bootnodes}`);
	}
	...
	let log = fs.createWriteStream(
		`node-${name}-${today.getFullYear()}-${today.getMonth()}-${today.getDate()}-${today.getTime()}.log`
	);
...
// Spawn a new validator node.
export function startValidators(
	bin: string,
	name: string,
	wsPort: number,
	port: number,
